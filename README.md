...
### ReadingBank (Reading Order Head Training)

Microsoft dataset with word-level reading order annotations (~500K pages).
Used to train visual reading order transformer head.
Download from [https://aka.ms/readingbank](https://aka.ms/readingbank).
...
Trained on ReadingBank. Uses RoI-pooled P3 features + transformer to predict reading order.
...
| `DOCLAYNET_DIR` | `PROJECT_ROOT/DocLayNet` | Detection training data |
| `READINGBANK_DIR` | `PROJECT_ROOT/ReadingBank` | Order training data |
